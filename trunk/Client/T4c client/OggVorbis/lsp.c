 ...
 ********************************************************************
  ...
  last mod: $Id: lsp.c 16037 2009-05-26 21:10:58Z xiphmont $
...
static int Laguerre_With_Deflation(float *a,int ord,float *r){
  ...
  double *defl=_alloca(sizeof(*defl)*(ord+1));
...
static int Newton_Raphson(float *a,int ord,float *r){
  int i, k, count=0;
  double error=1.f;
  double *root=_alloca(ord*sizeof(*root));
...
int vorbis_lpc_to_lsp(float *lpc,float *lsp,int m){
  ...
  float *g1=_alloca(sizeof(*g1)*(order2+1));
  float *g2=_alloca(sizeof(*g2)*(order2+1));
  float *g1r=_alloca(sizeof(*g1r)*(order2+1));
  float *g2r=_alloca(sizeof(*g2r)*(order2+1));
  int i;
