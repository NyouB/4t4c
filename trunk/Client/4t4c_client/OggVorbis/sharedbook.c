...
last mod: $Id: sharedbook.c 16073 2009-06-02 23:26:24Z xiphmont $
...
ogg_uint32_t *_make_words(long *l,long n,long sparsecount){//keep
	long i,j,count=0;
...
long _book_maptype1_quantvals(const static_codebook *b){//keep
...
float *_book_unquantize(const static_codebook *b,int n,int *sparsemap){//keep
	long j,k,count=0;
...
void vorbis_staticbook_clear(static_codebook *b){//keep
	if(b->allocedp){
