...
last mod: $Id: info.c 16077 2009-06-03 05:10:49Z xiphmont $
...
void vorbis_comment_add(vorbis_comment *vc,const char *comment)
{
	...
	strcpy_s(vc->user_comments[vc->comments],vc->comment_lengths[vc->comments]+1, comment);
	vc->comments++;
...
{
	unsigned long Size=strlen(tag)+strlen(contents)+2;
	char *comment=_alloca(Size); /* +2 for = and \0 */
	strcpy_s(comment,Size, tag);
	strcat_s(comment,Size, "=");
	strcat_s(comment,Size, contents);
...
char *vorbis_comment_query(vorbis_comment *vc, const char *tag, int count){
	...
	const unsigned long Size=taglen+ 1;
	char *fulltag = _alloca(Size);

	strcpy_s(fulltag,Size, tag);
	strcat_s(fulltag,Size, "=");
