 ...
 ********************************************************************
 ...
 last mod: $Id: psy.c 16037 2009-05-26 21:10:58Z xiphmont $
                                  ...
                                  float center_boost, float center_decay_rate)
{
  ...
  for(i=0;i<P_BANDS;i++){
    ...
    bin=(int)floor(fromOC(i*.5f)/binHz);
					   ...
					   int linesper,float dBoffset)
{
	...
	curve=posts+2;
	post1=(int)posts[1];
	seedptr=oc+((int)posts[0]-EHMER_OFFSET)*linesper-(linesper>>1);
...
static void bark_noise_hybridmp(int n,const long *b,
  ...
  float *N=_alloca(n*sizeof(*N));
  float *X=_alloca(n*sizeof(*N));
  float *XX=_alloca(n*sizeof(*N));
  float *Y=_alloca(n*sizeof(*N));
  float *XY=_alloca(n*sizeof(*N));
                               ...
                               float **mags)
{
  ...
  if(p->vi->normal_point_p)
  {
    ...
    float **work=_alloca(sizeof(*work)*partition);
...
void hf_reduction(vorbis_info_psy_global *g,
  ...
  int i,j,n=p->n, de=(int)(0.3*p->m_val);
  int limit=g->coupling_pointlimit[p->vi->blockflag][PACKETBLOBS/2];
  
  for(i=0; i<vi->coupling_steps; i++){
    /* for(j=start; j<limit; j++){} // ???*/
    for(j=limit; j<n; j++) 
      mdct[i][j] *= (float)(1.0 - de*((float)(j-limit) / (float)(n-limit)));
  }
}
